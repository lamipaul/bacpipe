...
class EmbeddingModel:
    ...
    def normalize_audio(
        ...
        framed_audio = np.divide(
            framed_audio, peak_norm, where=(peak_norm > 0.0)
        )
...
def batch_embed_from_embed_fn(
    ...
    for k in ["embeddings", "separated_audio", "frontend"]:
        if getattr(outputs[0], k) is not None:
            batched_outputs[k] = np.stack(
                [getattr(x, k) for x in outputs], axis=0
            )
        else:
