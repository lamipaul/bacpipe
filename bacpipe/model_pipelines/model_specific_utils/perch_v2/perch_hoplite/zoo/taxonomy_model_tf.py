...
class TaxonomyModelTF(zoo_interface.EmbeddingModel):
        ...
        for csv_path in csv_glob:
            ...
            with csv_path.open("r") as f:
                try:
                    ...
                    logging.warning(
                        "Failed to load class list %s: %s", csv_path, e
                    )
                    continue
        return class_lists
    ...
    def from_config(cls, config: config_dict.ConfigDict) -> "TaxonomyModelTF":
        ...
        if (
            hasattr(config, "tfhub_version")
            and config.tfhub_version is not None
        ):
        ...
        if (base_path / "saved_model.pb").exists() and (
            ...
            class_lists_glob = (epath.Path(model_path) / "assets").glob(
                "*.csv"
            )
        else:
            ...
            for vname, vshape in tf.train.list_variables(vars_filepath):
                if len(vshape) == 1 and vshape[-1] == num_classes:
                    if bias is None:
                        ...
                        raise ValueError(
                            "Multiple possible biases for class list."
                        )
                if len(vshape) == 2 and vshape[-1] == num_classes:
                    if weights is None:
                        ...
                        raise ValueError(
                            "Multiple possible weights for class list."
                        )
