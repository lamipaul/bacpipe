...
from bacpipe.model_pipelines.model_specific_utils.perch_v2.perch_hoplite.zoo import (
    hub,
)
from bacpipe.model_pipelines.model_specific_utils.perch_v2.perch_hoplite.zoo import (
    zoo_interface,
...
def _get_obj(module, attr_name):
    ...
    if hasattr(module, attr_name):
        ...
        raise ValueError(
            f"Module {module} does not have attribute {attr_name}"
        )
...
def get_model_class(model_key: str) -> type[zoo_interface.EmbeddingModel]:
    ...
    if model_key in (
        ...
        module = importlib.import_module(
            "bacpipe.model_pipelines.model_specific_utils.perch_v2.perch_hoplite.zoo.taxonomy_model_tf"
        )
        return _get_obj(module, "TaxonomyModelTF")
    elif model_key == "google_whale":
        module = importlib.import_module(
            "bacpipe.model_pipelines.model_specific_utils.perch_v2.perch_hoplite.zoo.models_tf"
        )
        return _get_obj(module, "GoogleWhaleModel")
    elif model_key == "placeholder_model":
        module = importlib.import_module(
            "bacpipe.model_pipelines.model_specific_utils.perch_v2.perch_hoplite.zoo.placeholder_model"
        )
        ...
        module = importlib.import_module(
            "bacpipe.model_pipelines.model_specific_utils.perch_v2.perch_hoplite.zoo.aves_model"
        )
        return _get_obj(module, "AVES")
    elif model_key == "handcrafted_features_model":
        module = importlib.import_module(
            "bacpipe.model_pipelines.model_specific_utils.perch_v2.perch_hoplite.zoo.handcrafted_features_model"
        )
...
def get_preset_model_config(preset_name: str | ModelConfigName) -> PresetInfo:
    ...
    if preset_name == ModelConfigName.PERCH_8:
        ...
        model_config.model_url = (
            "google/humpback-whale/tensorFlow2/humpback-whale"
        )
        ...
        model_config.model_url = (
            "google/multispecies-whale/tensorFlow2/default"
        )
        ...
        if birdnet_version not in ("V2.1", "V2.2", "V2.3"):
            raise ValueError(
                f"Birdnet version not supported: {birdnet_version}"
            )
